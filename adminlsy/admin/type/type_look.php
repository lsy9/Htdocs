...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  <tr>
    <td width="99%" align="left" valign="top">您的位置：版块管理&nbsp;&nbsp;>&nbsp;&nbsp;查看帖子</td>
  </tr>
  <tr>
    <td align="left" valign="top" id="addinfo">
    <a href="./type_menu.php" target="mainFrame" onFocus="this.blur()" class="add">返回上一级</a>
    </td>  <!---->
  </tr>
  <tr>
	  ...
	  <?php
			...
			//准备SQL语句
			$psql = "select id,uid,tid,title,content,ctime,count from post where tid={$id}";
		...
		<?php
			...
			//准备 SQL 语句
			//先查询出子版块的父类id即子版块的pid（查询主版块的第一步），name为子版块的名字		
			$tsql = "select name,pid from type where id={$parray['tid']}";
			
			//执行SQL语句
			$tresult = mysqli_query($link,$tsql);
			
			//获取结果集
			@$tarray = mysqli_fetch_assoc($tresult);
			...
			//准备SQL语句
			//根据查询出来的子版块pid查询所属主版块
			$fraTypeSQL = "select name from type where id={$tarray['pid']}";
			
			//执行SQL语句
			$fraTypeResult = mysqli_query($link,$fraTypeSQL);
			
			//获取结果集
			@$fraTypeArray = mysqli_fetch_assoc($fraTypeResult);
		...
		<td align="left" valign="top">
			<table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
			  ...
			  <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
				...
				<td align="left" valign="middle" class="borderright borderbottom main-for"><?php echo "{$parray['uid']}" ?></td>
				</tr>
